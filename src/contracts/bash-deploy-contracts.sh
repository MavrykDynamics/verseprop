...
for contract_test in "${CONTRACTS_DEPLOY_ARRAY[@]}"; do
    case "$contract_test" in
        ...
        kyc)
            ...
            COMMANDS+=("yarn ts-mocha --paths test/deploy/09_deploy_super_admin.spec.ts --bail --timeout 9000000")
            COMMANDS+=("yarn ts-mocha --paths test/deploy/10_deploy_kyc.spec.ts --bail --timeout 9000000")
            COMMANDS+=("yarn ts-mocha --paths test/deploy/01c_deploy_rwa_token.spec.ts --bail --timeout 9000000")
            ;;
        superAdmin)
        ...
        debtController)
            ...
            COMMANDS+=("yarn ts-mocha --paths test/deploy/01_deploy_debt_controller.spec.ts --bail --timeout 9000000")
            ;;
        all)
