...
for contract_test in "${CONTRACTS_DEPLOY_ARRAY[@]}"; do
    case "$contract_test" in
        ...
        superAdmin)
            ...
            COMMANDS+=("yarn ts-mocha --paths test/deploy/01_deploy_super_admin.spec.ts --bail --timeout 9000000")
            ;;
        kyc)
            echo "Deploying KYC Contract"
            COMMANDS+=("yarn ts-mocha --paths test/deploy/02_deploy_kyc.spec.ts --bail --timeout 9000000")
            ;;
        rwaToken)
            echo "Deploying RWA Token"
            COMMANDS+=("yarn ts-mocha --paths test/deploy/03_deploy_rwa_token_non_fungible.spec.ts --bail --timeout 9000000")
            ;;
        debtController)
            echo "Deploying Debt Controller"
            COMMANDS+=("yarn ts-mocha --paths test/deploy/04_deploy_debt_controller.spec.ts --bail --timeout 9000000")
            ;;
        all)
